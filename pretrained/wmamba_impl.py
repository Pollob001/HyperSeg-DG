...
class SwinMambaLayer(nn.Module):
    """depth × SwinMambaBlock, then optional PatchMerging2D."""
    def __init__(self, dim, depth, input_resolution: Tuple[int, int], window_size=7,
        ...
        for i in range(depth):
            ...
            self.blocks.append(SwinMambaBlock(
...
class SwinMamba(nn.Module):
    """
    Swin hierarchy with Mamba blocks:
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        for i in range(4):
            depth_i = depths[i]
            layer = SwinMambaLayer(self.dims[i], depth_i, (H, W), window_size,
...
def swinmamba_t(img_size=256, num_classes=1000, **kwargs) -> SwinMamba:
    return SwinMamba(
        img_size=img_size, num_classes=num_classes,
        depths=(2, 2, 4, 2), dims=(96, 192, 384, 768), window_size=8,
        **kwargs
    )

def swinmamba_s(img_size=256, num_classes=1000, **kwargs) -> SwinMamba:
    return SwinMamba(
        img_size=img_size, num_classes=num_classes,
        depths=(2, 2, 8, 2), dims=(96, 192, 384, 768), window_size=8,
        **kwargs
    )

def swinmamba_b(img_size=256, num_classes=1000, **kwargs) -> SwinMamba:
    return SwinMamba(
        img_size=img_size, num_classes=num_classes,
        depths=(2, 2, 12, 2), dims=(128, 256, 512, 1024), window_size=8,
        **kwargs
    )
...
def wmamba_t(img_size=256, num_classes=1000, **kwargs) -> WMamba:
    return swinmamba_t(img_size=img_size, num_classes=num_classes, **kwargs)


def wmamba_s(img_size=256, num_classes=1000, **kwargs) -> WMamba:
    return swinmamba_s(img_size=img_size, num_classes=num_classes, **kwargs)


def wmamba_b(img_size=256, num_classes=1000, **kwargs) -> WMamba:
    return swinmamba_b(img_size=img_size, num_classes=num_classes, **kwargs)


if __name__ == "__main__":
    device = "cuda" if torch.cuda.is_available() else "cpu"
    x = torch.randn(2, 3, 256, 256).to(device)
    model = swinmamba_t(num_classes=6, d_state=16).to(device)
    with torch.no_grad():
        y = model(x)
